...
def add_official_summary(document, assets):
    ...
    rows = [
        ...
        (
            "제품\n(아이디어)\n소개",
            "충북 외국인 주민의 다국어 질문, OCR 신청서 상태, 공식 공고 일정을 분석하여 비자정보·보완서류·행정일정·전문기관 중 적절한 다음 행동으로 연결하는 모바일 우선 웹 서비스이다. AI 챗봇은 질문의 의도·비자·지역·위험 신호를 파악하고, OCR은 서류를 완성·확인 필요·누락·수동 확인으로 구분한다. 자격·점수와 날짜는 검수된 데이터와 결정론적 규칙으로 계산하며, 준비가 끝나면 비자포털·하이코리아 등 공식 채널로 연결한다.",
        ),
        (
            "제품\n(아이디어)\n사업성",
...
def build(output_path):
    ...
    document.add_heading("2-4. 공식 서비스와 남은 준비 구간", level=2)
    add_standard_table(
        document,
        ["기존 서비스·방식", "주요 역할", "비자부기가 보완하는 지점"],
        [
            ["대한민국 비자포털 [14]", "비자 종류·공통 서류 안내, 전자비자 신청·결과 조회", "충북 공고 차수별 요건을 개인의 준비 행동으로 변환"],
            ["하이코리아 [11]", "국내 체류 전자민원·방문예약·조회", "민원 전에 필요한 요건·서류·일정을 지속해서 추적"],
    ...
    add_caption(document, "표 2. 공식 서비스와 비자부기의 역할 및 보완 범위 [10][11][14]")
    add_body(
        document,
        "대한민국 비자포털과 하이코리아는 비자 안내, 공식 신청, 체류 민원과 결과 조회를 담당한다. 비자부기는 공식 채널에 접속하기 전 충북 공고와 붙임서식을 개인별 요건, OCR 서류 상태, 공고 일정, 지원기관으로 변환해 준비 과정을 관리한다. 제출과 판정은 공식기관에서 이루어진다.",
        after=0,
    )
    ...
    add_body(
        document,
        "비자부기의 핵심 사용자는 충북에서 취업과 정착을 준비하는 외국인 유학생과 체류자격 전환을 검토하는 외국인 근로자다. 외국인을 채용하는 기업 담당자, 대학 국제교류부서, 가족센터, 외국인지원센터, 지자체 상담 담당자도 같은 정보를 확인하는 기관 사용자로 설정했다.",
    )
    add_body(
        document,
        "대표적인 사용 상황은 충북 소재 기업에 취업한 외국인 유학생이 F-2-R 전환을 준비하거나, E-9 근로자가 E-7-4R 전환 가능성을 확인하는 경우다. 사용자는 편한 언어로 자신의 체류자격과 거주지역, 현재 상황을 입력한다. 공고문이나 신청서를 촬영해 올릴 수도 있다.",
    )
    add_body(
        document,
        "AI 챗봇은 질문에서 비자 유형과 지역, 현재 준비 단계, 위험 신호를 파악한다. 임금체불이나 산업재해처럼 즉시 대응이 필요한 내용이 발견되면 해당 지역의 전문기관 연락처를 우선 안내한다. 일반적인 비자 문의에는 검수된 요건과 제출서류를 조회하고, OCR 결과와 함께 부족한 항목을 보여준다.",
    )
    add_body(
        document,
        "접수기간이나 제출기한이 확인된 경우에는 캘린더에 등록할 수 있다. 날짜의 근거가 부족하면 임의로 계산하지 않고 확인이 필요한 항목으로 남긴다. 사용자는 준비 현황과 다음 할 일을 홈 화면에서 확인하며, 필요한 경우 공식 신청기관이나 충북 지역 지원기관으로 연결된다.",
    )
    document.add_heading("사용 경험의 원칙", level=3)
    add_body(
        document,
        "로그인 전 입력한 정보는 현재 브라우저 세션에서만 사용한다. 자격요건이 불명확하거나 담당기관의 판단이 필요한 경우에는 결과를 ‘검토 필요’로 표시한다. 각 안내에는 출처 문서와 적용기간, 마지막 검증일을 함께 제공한다.",
        after=0,
    )
    ...
    add_body(
        document,
        "비자부기는 충북 공고를 사용자의 체크리스트와 OCR 결과, 캘린더 일정으로 바꾸어 신청 전 준비를 돕는다. 준비가 끝나면 비자포털·하이코리아 또는 관할기관의 공식 절차로 연결한다.",
    )
    ...
    add_caption(document, "그림 2. 공식 문서에서 서비스까지 이어지는 4계층 데이터 구조")
    add_standard_table(
        document,
    ...
    add_body(document, "비자 챗봇은 의도와 위험 신호, 사용 언어를 분류하고 쉬운 표현으로 설명한다. 요건과 절차, 제출서류, 기관 조회 범위는 검수된 데이터로 제한한다.", size=9.2, after=3)
    add_body(document, "OCR 도우미는 선택한 필드의 의미를 설명한다. 서식 템플릿과 작성 주체, 누락 상태는 정해진 규칙으로 판정한다.", size=9.2, after=3)
    add_body(document, "공고 일정은 사용자 상황에 맞춰 안내한다. 날짜 계산과 변경 비교에는 확정일, 기준일, 공식 offset만 사용한다.", size=9.2)
    add_body(
        document,
    ...
    add_caption(document, "그림 3. 비자 여정 단계별 서비스 책임 경계")
    add_compact_flow(
        document,
        [
            ("1. 추적", "요건·진행 현황\n비자부기 지원"),
    ...
    add_caption(document, "그림 4. 이주노동자의 E-7-4R 준비와 위험상황 연결")
    add_compact_flow(
        document,
        [
    ...
    add_body(
        document,
        "AI가 비자 문의와 임금체불 신호를 함께 파악하면 지역과 사용자 유형에 맞는 노동 전문기관을 먼저 안내한다. 이후 검수된 E-7-4R 요건, OCR 서류 상태, 공고 마감일을 체크리스트와 캘린더에 반영한다.",
        size=9.2,
    )
    ...
    add_caption(document, "그림 5. 유학생의 유학·취업·정착 준비 과정")
    add_compact_flow(
        document,
        [
            ("1. AI 질문 이해", "학적·비자·지역\n문의 의도 분류"),
    ...
    add_body(
        document,
        "OCR은 작성 주체와 필수 여부, 누락, 수동 확인 항목을 구분한다. 챗봇은 실제 개인정보 대신 필드 상태를 이용해 작성 방법을 설명한다. 누락서류는 체크리스트에 표시하고, 확인된 마감일은 캘린더에 등록한다.",
        size=9.2,
    )
    ...
    add_standard_table(
        document,
        ["비교 항목", "비자포털·하이코리아", "Study in Chungbuk", "비자부기"],
        [
            ["핵심 역할", "비자 안내·공식 신청·체류 민원·조회", "유학생 정보·다국어 안내", "충북 공고 기반 신청 전 준비·추적"],
            ["정보 단위", "전국 공통 체류자격·공통 최소서류", "유학생 지원정보·콘텐츠", "충북 공고 차수·적용기간·지역요건"],
            ["자격 확인", "체류자격별 일반 신청요건", "정보·번역 중심", "AND/OR·점수·필수기준을 사용자 입력과 대조"],
            ["서류 지원", "공식 서식·공통 제출서류 안내", "콘텐츠 안내", "OCR 필드 상태·작성 주체·첨부관계 점검"],
            ["일정 지원", "공식 절차·처리 현황 확인", "정보 탐색 중심", "공고·기준일을 캘린더 일정과 할 일로 등록"],
            ["책임 경계", "공식 접수·민원·심사", "정보와 관련 링크 제공", "준비를 지원한 뒤 공식기관으로 연결"],
        ],
        [1900, 2440, 2440, 3300],
        font_size=8.4,
        first_col_bold=True,
    )
    add_caption(document, "표 3. 공식 서비스와 비자부기의 역할 비교 [10][11][14]")
    ...
    add_caption(document, "표 4. 2026년 8월 26일 팀 데이터 저장소 검증 스냅샷 [6]")
    add_body(
        document,
        "비자부기의 경쟁력은 충북 공고와 붙임서식을 계산 가능한 규칙으로 만들고 이를 사용자의 체크리스트, OCR 결과, 캘린더와 연결하는 데 있다. AI는 위험 대응, 비자정보, 서류 점검, 일정 관리 중 필요한 경로를 고른다. 공식 포털과 하이코리아는 최종 신청과 민원을 담당하고, 비자부기는 준비가 끝난 사용자를 해당 공식 채널로 넘긴다.",
    )
    add_body(
        document,
    ...
    add_body(
        document,
        "개인 사용자에게는 비자 요건 확인과 기본 체크리스트, 일정, 기관 검색을 무료로 제공한다. 서류 묶음 관리와 가족 공동 준비, 고급 알림은 실제 수요와 법률·개인정보 검토를 거쳐 선택형 기능으로 확장한다.",
        size=9.4,
    )
    add_body(
        document,
        "대학 국제교류부서와 외국인지원센터, 기업 인사담당자, 지자체에는 기관별 체크리스트와 공고 변경 알림, 담당자 검수, 비식별 집계, 근거 링크를 제공한다. 기관용 연간 사용료로 AI 처리비와 공식 행정데이터의 검수·갱신·운영비를 충당한다.",
        size=9.4,
    )
    add_body(
        document,
        "초기에는 충북 지역특화형 비자와 유학생 체류·취업 안내에 집중한다. 데이터 갱신과 기관 운영 방식을 검증한 뒤 타 지자체의 지역특화형 비자와 전국 공통 체류자격으로 범위를 넓힌다.",
        size=9.4,
    )
    add_standard_table(
        document,
    ...
    add_body(
        document,
        "대학과 지원기관은 같은 공식 출처와 검증일을 보면서 상담할 수 있다. 원문이 충돌하거나 담당기관의 판단이 필요한 사례는 별도로 표시한다. 비식별 집계 자료를 활용하면 어느 단계에서 문의가 반복되는지 파악해 안내자료와 상담 인력, 지원사업을 조정할 수 있다.",
    )
    add_body(
        document,
    ...
    add_body(
        document,
        "외국인 주민이 이해할 수 있는 언어로 필요한 절차를 확인하면 행정 정보격차를 줄일 수 있다. 신청 지연과 기관 재방문, 서류 재발급에 드는 시간과 비용도 함께 줄어든다.",
    )
    add_body(
        document,
        "안정적인 비자 전환은 외국인 인재의 취업 지속과 지역 기업의 인력난 완화에 도움이 된다. 모든 결과에 출처와 적용기간, 확인이 필요한 부분을 표시하는 방식은 공공 분야에서 AI를 책임 있게 활용하는 사례로도 확장할 수 있다.",
    )
    ...
    add_source(
        document,
        14,
        "비자 내비게이터·전자비자 신청·비자발급인정서 안내",
        "대한민국 비자포털(법무부)",
        "2026년 확인",
        "https://www.visa.go.kr/openPage.do?MENU_ID=1020502",
    )
    add_body(
        document,
