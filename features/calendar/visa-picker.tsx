  ...
  return (
    <div>
      <details className="group relative">
        <summary
          aria-label={t("label")}
          className="flex min-h-12 cursor-pointer list-none items-center justify-between gap-4 rounded-2xl border border-[#d6dfda] bg-white px-4 py-2 text-left shadow-sm marker:content-none focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[#2d6d5d] [&::-webkit-details-marker]:hidden"
        >
          <span className="min-w-0">
            {selectedVisas.length ? (
              <span className="flex flex-wrap gap-2 text-[14px] leading-5 font-normal text-[#30433b]">
              ...
              <span className="text-[14px] leading-5 font-normal text-[#77837e]">{t("placeholder")}</span>
            )}
          </span>
          <span className="flex shrink-0 items-center gap-2 text-[14px] leading-5 font-normal text-[#5d6d66]">
        ...
        <div className="absolute z-20 mt-2 w-full overflow-hidden rounded-2xl border border-[#d6dfda] bg-white p-2 shadow-[0_14px_36px_rgba(40,63,53,0.14)]" role="group" aria-label={t("optionsAriaLabel")}>
          {selectedVisaIds.length ? (
            <div className="mb-1 flex justify-end border-b border-[#edf0ee] px-1 pb-2">
              <button type="button" onClick={onClear} className="min-h-10 rounded-lg px-3 text-[14px] leading-5 font-normal text-[#52615b] hover:bg-[#f4f7f4] focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[#2d6d5d]">
                {t("clear")}
              </button>
            </div>
          ) : null}
          {SUPPORTED_VISAS.map((option) => (
            <button
              ...
              className={`flex min-h-12 w-full items-center justify-between gap-4 rounded-xl px-3 py-2 text-left text-[14px] leading-5 font-normal transition-colors ${
                selectedVisaIds.includes(option.id)
                  ? "bg-[#e9f3ef] text-[#1f584a]"
                  : "text-[#33453e] hover:bg-[#f4f7f4]"
              }`}
            >
              <span className="text-[14px] leading-5 font-normal">{option.id} <span aria-hidden="true">|</span> {t(`visas.${option.messageKey}`)}</span>
