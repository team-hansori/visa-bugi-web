...
def build(output_path):
    ...
    add_body(
        document,
        "실제 구축 과정에서는 E-7-4R 공고의 벌금 감점 기준이 본문·점수표와 서식에서 서로 다르게 남아 있는 충돌과 HWPX의 빈 개체로 PDF와 원문 페이지가 어긋나는 사례를 확인했다. 비자부기는 이런 경우 두 근거를 모두 보존하고 자동 판정을 차단한다.",
    )
