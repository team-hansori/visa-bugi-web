...
def build(output_path):
    ...
    add_caption(document, "도식 3. 공식 문서를 서비스 정보로 전환하는 데이터 신뢰 구조")
    add_standard_table(
        document,
        ["단계", "처리 대상", "검증 기준", "서비스 산출"],
        [
            ["1. 수집", "공고·지침·서식", "문서·페이지·적용기간 보존", "원문 근거 묶음"],
            ["2. 추출", "본문·표·각주·붙임", "원문 대조와 충돌·미확인 기록", "검수 대기 데이터"],
            ["3. 구조화", "조건·절차·서류·출처", "관계·기간·무결성 검증", "13개 관계형 테이블"],
            ["4. 제공", "규칙·OCR·다국어·기관", "승인된 데이터만 소비", "요건 결과와 다음 행동"],
        ],
        [1450, 2650, 3330, 2650],
        font_size=8.8,
        first_col_bold=True,
    ...
    add_standard_table(
        ...
        [
            ["핵심 역할", "공식 민원 신청·조회", "유학생 정보·다국어 안내", "신청 전 준비·추적"],
            ["개인별 요건", "공고·절차를 사용자가 대조", "정보 탐색 중심", "검수된 규칙으로 계산"],
            ["AND/OR·점수", "공식 문서에 분산", "개별 정보 안내", "논리 트리·점수 모델"],
            ["차수·근거", "현행 공식 절차 제공", "포털 콘텐츠 제공", "변경이력·페이지·적용기간 연결"],
            ["행동 관리", "민원 접수·조회 중심", "정보 탐색 중심", "서류·일정·기관 통합"],
            ["책임 경계", "공식 접수·판정", "지역 유학생 정보 채널", "준비 지원 후 공식기관으로 이관"],
        ],
        [1900, 2440, 2440, 3300],
        font_size=8.4,
        first_col_bold=True,
    ...
    add_standard_table(
        ...
        ["역할", "수행 역량과 근거"],
        [
            ["행정 데이터", "공식 PDF·HWP 원문 추출, 사람 검수, 13개 관계형 테이블과 684개 근거 매핑 운영"],
    ...
    add_note_box(
        document,
        "수익모델 검증 원칙",
        "가격 구간과 기관 목표는 현재 매출·실적이 아닌 검증 가설이다. 현장 인터뷰와 실증에서 기능 범위·지불의향을 확인한 뒤 확정한다.",
    )
