...
def build(output_path):
    ...
    add_body(
        document,
        "외국인 주민의 비자 준비에는 세 가지 정보격차가 있다. 자신의 상황을 어떤 비자 요건과 연결해야 하는지 모르는 질문 격차, 신청서의 작성자와 보완항목을 구분하기 어려운 서류 격차, 공고의 접수기간과 개인별 행정기한을 추적하기 어려운 시간 격차다.",
    )
    add_body(
        document,
        "정보는 PDF·HWPX·HWP의 본문·표·각주·붙임서식에 분산되어 있다. 질문에 답을 얻어도 서류 누락이나 마감일을 놓치면 신청을 진행할 수 없다. 비자부기는 AI 챗봇·OCR·공고 스케줄링을 연결해 질문을 실제 행정 행동으로 변환한다.",
    )
    add_metric_strip(
        document,
        [
    ...
    add_body(
        document,
        "2025년 충북의 장기체류외국인은 86,490명이며, 이 가운데 E계열 취업 외국인은 27,705명이다. 2026년 4월 충북 외국인 유학생도 14,303명에 이른다. 충북은 F-2-R·E-7-4R 등 지역특화형 비자를 반복 공고하고 있으며, 같은 해 유학생 채용박람회에는 유학생 약 700명과 도내 기업 32개사가 참여했다. 인구감소지역 소재 13개 기업은 F-2-R 전환 희망 유학생과 현장 면접을 진행했다. 취업·비자 전환·정착이 하나의 지역 사용자 여정이라는 근거다. [2][3][8][9]",
    )
    add_note_box(
        document,
        "충북 현안과의 연결",
    ...
    add_body(
        document,
        "팀은 화면부터 만들지 않고 구축 대상 공식 원천문서 24건 전체를 먼저 파싱했다. PDF·HWPX·HWP의 본문·표·각주·붙임서식을 추출한 뒤, 원문 계층→근거 계층→규칙 계층→서비스 계층의 4단계 데이터 구조를 구성했다. 사람이 원문과 대조한 자격요건·점수표·절차·서류·쿼터·변경이력만 13개 관계형 테이블로 이관하고, 684개 원천→서비스 근거 매핑으로 추적 가능하게 연결했다.",
    )
    ...
    add_body(
        document,
        "대표 시나리오는 충북 소재 기업에 취업한 외국인 유학생이 F-2-R 전환을 검토하거나, E-9 근로자가 E-7-4R 전환을 준비하는 과정이다. 비자부기는 질문 이해→서류 점검→공고 일정 연결→위험상황 기관 연결을 하나의 AI 사용자 여정으로 제공한다.",
    )
    ...
    add_compact_flow(
        ...
        "AI는 경로를 선택하고 자격·연락처·행정일정은 검수된 데이터와 코드가 통제",
    )
    ...
    add_standard_table(
        ...
        [
            ...
            ["공고 스케줄링", "접수기간·절차·개인 기준일을 캘린더 행동으로 변환", "근거 없는 날짜를 생성하지 않음"],
    ...
    add_body(
        document,
        "핵심 원칙은 ‘AI는 무엇을 물었는지 파악하고, 무엇을 답할지는 검수된 데이터가 결정한다’는 역할 분리다. 구축 대상 공식 원천문서 24건 전체를 파싱해 원문·근거·규칙·서비스의 4계층으로 구조화했으며, 모든 질문은 위험 신호·사용자 유형·지역·비자·언어로 분류된 뒤 제한형 조회와 결정론적 코드로 처리된다.",
    )
    add_caption(document, "도식 3. 질문·OCR·공고를 행동으로 연결하는 AI 라우팅 구조")
    add_standard_table(
        document,
        ["단계", "입력·처리", "AI 역할", "결정론적 통제"],
        [
            ["1. 입력", "다국어 질문·OCR 상태·공고", "질문의 맥락과 언어 이해", "입력 형식·길이·필드 검증"],
            ["2. 스크리닝", "위험·사용자·지역·비자", "완곡한 표현을 포함한 의도 분류", "고정된 유형과 스키마로 제한"],
            ["3. 경로 선택", "위험·비자·서류·일정", "필요한 정보 경로 선택", "검수 테이블과 제한형 조회 실행"],
            ["4. 행동 출력", "설명·체크리스트·캘린더·기관", "쉬운 다국어 설명", "출처·적용기간·연락처·날짜 검증"],
        ],
        [1450, 2650, 3330, 2650],
        font_size=8.5,
        first_col_bold=True,
    ...
    add_standard_table(
        document,
        ["기능", "AI가 담당하는 일", "코드·데이터가 담당하는 일"],
        [
            ["비자 챗봇", "의도·위험·언어 분류와 설명", "유효한 요건·절차·서류·기관 조회"],
            ["OCR 도우미", "선택 필드의 쉬운 설명", "템플릿·작성 주체·누락 상태 판정"],
            ["공고 일정", "사용자 상황에 맞는 일정 안내", "확정일·기준일·offset 계산과 변경 비교"],
        ],
        [2200, 3500, 4380],
        font_size=8.9,
        first_col_bold=True,
    ...
    add_body(
        document,
        "OCR은 신청서를 완성·확인 필요·누락·수동 확인으로 나누고 작성 주체를 구조화한다. 공고 스케줄러는 접수 시작일·마감일·절차·적용기간을 목표 비자와 연결한다. 확정일은 그대로 표시하고 개인 기준일형 일정은 기준일 확인 후 계산하며, 공고 변경 시 이전 일정과 달라진 근거를 함께 보여준다.",
    )
    ...
    add_standard_table(
        ...
        [
            ...
            ["일정 지원", "공식 절차별 확인", "정보 탐색 중심", "공고·기준일을 캘린더 행동으로 변환"],
    ...
    add_body(
        document,
        "비자부기의 독창성은 문서 요약형 챗봇이 아니라, 질문·OCR 서류 상태·공고를 함께 해석해 위험·비자정보·서류·일정 중 필요한 경로를 선택하고 결과를 실제 행동으로 전환하는 AI 라우팅 구조에 있다.",
    )
    add_note_box(
        document,
        "심사기준 대응",
        "독창성: 질문·OCR·공고를 행동으로 바꾸는 AI 라우팅. 기술성: 공식 원천문서 24건 전체 파싱, 4계층 데이터 구조, 결정론적 검증. 사업화: 기관형 수익모델과 정량 실증 목표. 적합성: 충북 외국인 주민의 비자 전환·정착 현안에 적용한 실제 시제품.",
    )
    ...
    add_compact_flow(
        ...
        "시제품 가점 대응: 반응형 웹에 AI 챗봇·OCR·캘린더·기관 지도를 실제 적용",
    )
    add_standard_table(
        document,
        ["구분", "현재 구현 내용"],
        [
            ["데이터", "공식 원천문서 24건 전체 파싱, 원문·근거·규칙·서비스 4계층, 13개 관계형 테이블과 684개 근거 매핑"],
            ["AI 챗봇", "의도·위험·사용자·지역·비자·언어 스크리닝, 제한형 데이터 조회, 다국어 답변"],
            ["위험 라우팅", "임금체불·산재·폭행·불법취업·거주조건 위반 분류, 사용자·지역별 기관 연결"],
            ["OCR 도우미", "서식 템플릿 분석, 필드별 누락·작성 주체·수동 확인, 6개 언어 질문 도우미"],
            ["공고 일정", "목표 비자별 접수기간 표시, 기준일·offset 계산, 일정 검색·개인 일정 등록"],
            ["안전·개인정보", "출처·연락처 원문 검증, OCR 비민감 필드 상태 활용, 대화 삭제와 비식별 운영 로그"],
        ],
        [2100, 7980],
        font_size=8.5,
        first_col_bold=True,
    ...
    add_note_box(
        document,
        "비용 구조와 충북 지역 환류",
        "기관이 소액의 AI 처리비와 데이터 운영비를 부담하면 외국인 주민은 비용 없이 서류 누락·공고 일정·지원기관을 확인한다. 이를 통해 채용 이후 비자 전환 과정의 이탈을 줄이고 충북 기업의 외국인 인력 유지와 지역 정착으로 편익을 환류한다. 가격·비용 수치는 현재 가설이며 실제 사용량과 현장 지불의향으로 확정한다.",
    )
    ...
    add_note_box(
        document,
        "최종 제안",
        "비자부기는 또 하나의 정보 페이지가 아니다. AI가 질문을 이해하고, OCR로 서류를 읽고, 공고 일정을 추적하며, 위험상황에는 전문기관으로 선제 연결한다. 구축 대상 공식 문서 전체를 근거·규칙·행동으로 전환한 충북 현안 해결형 AI 비자·정착 디지털 인프라다.",
    )
