...
export { resolveStoredTargetVisaCode as resolveTargetVisaId } from "@/lib/onboarding/target-visa";

/**
 * 목표 비자는 온보딩이 user_visa_profile에 저장한 값을 사용한다.
 * 아직 세션이나 저장된 값이 없으면 빈 배열로 시작해 전체 일정을 보여준다.
 */
export function useTargetVisaIds(): {
  targetVisaIds: string[];
  toggleVisaId: (visaId: string) => void;
  clearVisaIds: () => void;
} {
  ...
  function toggleVisaId(visaId: string) {
    setManualVisaIds((current) => {
      ...
      return selected.includes(visaId)
        ? selected.filter((id) => id !== visaId)
        : [...selected, visaId];
    });
  }
